...
public final class GdxGameUtils {
    ...
    public static void drawTextureRegionScaled(Batch batch, TextureRegion region, Rectangle rectangle, float rotation, float scale) {
        int regionWidth = region.getRegionWidth();
        int regionHeight = region.getRegionHeight();
        float regionOriginX = regionWidth / 2f;
        float regionOriginY = regionHeight / 2f;
        batch.draw(region, rectangle.x, rectangle.y, regionOriginX, regionOriginY, regionWidth, regionHeight, scale, scale, rotation);
    }
